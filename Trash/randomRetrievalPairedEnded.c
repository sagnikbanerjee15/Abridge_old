...
int main(int argc, char *argv[])
{
	...
	int split_on_newline_qual_ROWS = ROWS * 10;
	int split_on_newline_qual_COLS = COLS * 100;
	int read_names_ROWS = -1;
	int read_names_COLS = -1;
	...
	strcpy(dictionary_name, argv[13]);
	...
	fhr_dictionary = fopen(dictionary_name, "rb");
	...
	split_on_newline_qual = (char**) malloc(
			sizeof(char*) * split_on_newline_qual_ROWS);
	for (i = 0; i < split_on_newline_qual_ROWS; i++)
		split_on_newline_qual[i] = (char*) malloc(
				sizeof(char) * split_on_newline_qual_COLS);
	...
	buffer_for_pass1 = (char*) malloc(
			sizeof(char) * MAX_BUFFER_SIZE_FOR_READING_PASS2_FILE);
	single_genome_sequence = (struct Whole_Genome_Sequence*) malloc(
	...
	first_record = findReadClusterFromAbridgeIndex(
			abridge_index,
			chromosome,
			start,
			end,
			&abridge_match_start_index,
			&abridge_match_end_index);
	writeSequenceHeaders(fhw, genome_filename, 0);

	buffer_for_qual = (char*) malloc(
			sizeof(char) * MAX_BUFFER_SIZE_FOR_READING_PASS2_FILE);
	from = start;
	to = end;
	/*
	...
	for (i = abridge_match_start_index; i <= abridge_match_end_index; i++)
	{
		...
		fread_ret_val = fread(
				buffer_for_pass1,
				1,
				abridge_index->end_byte[i] - abridge_index->start_byte[i],
				fhr_pass1);
		...
		number_of_entries_in_cluster = splitByDelimiter(
				buffer_for_pass1,
				'\n',
				split_on_newline);
		...
		read_names_ROWS = max_reads + 100;
		read_names_COLS = 100;
		read_names = (char**) malloc(sizeof(char*) * read_names_ROWS);
		for (j = 0; j < read_names_ROWS; j++)
		...
		fseek_ret_val = fseek(
				fhr_qual,
				abridge_index->start_byte_qual[i],
				SEEK_SET);
		fread_ret_val = fread(
				buffer_for_qual,
				1,
				abridge_index->end_byte_qual[i]
						- abridge_index->start_byte_qual[i],
				fhr_qual);
		/*
		...
		split_on_newline_qual = (char**) malloc(
				sizeof(char*) * split_on_newline_qual_ROWS);
		for (j = 0; j < split_on_newline_qual_ROWS; j++)
			split_on_newline_qual[j] = (char*) malloc(
					sizeof(char) * split_on_newline_qual_COLS);

		total_quals = splitByDelimiter(
				buffer_for_qual,
				'\n',
		...
		for (j = 0; j < number_of_entries_in_cluster; j++)
		{
			...
			for (k = 0; k < number_of_distinct_cigars_in_a_line; k++)
			{
				...
				else
				{
					...
					{
						sam_alignment->mapping_quality_score = strtol(
								split_on_dash[1],
								&temp,
								10);
				...
				if (curr_position > end || curr_position < start)
					continue;
				if (split_on_comma[k][1] == '-'
						&& isalpha(split_on_dash[0][0]) != 0)
				{
					// Use the same cigar
				...
				while (number_of_repititions_of_the_same_reads--)
				{
					...
					printf("%s", read_names[read_names_index]);
					read_names_index++;
					printf("\t");
					...
					if (flag_save_all_quality_scores == 1)
					{
						...
						printf(
								"%s",
								split_on_newline_qual[qual_pool_iterator++]);
					}
					else
						printf("%s", sam_alignment->qual);
					printf("\t");
					/*
					 printf ("\nWritten upto here");
					 fflush (stdout);
