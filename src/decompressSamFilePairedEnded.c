		...
		int max_reads_in_each_line)
{
	...
	if ( fhr_qual == NULL )
	{
		printf ("3. Error! File not found %s" ,
	...
	if ( fhw == NULL )
	{
		printf ("4. Error! File %s cannot be opened for writing" ,
				output_sam_filename);
		exit (1);
	}
	...
	number_of_unique_samformatflags = fillUpDictionary (samflag_dictionary ,
			fhr_dictionary ,
			10000);
	...
	while ( ( line_len = getline ( &buffer , &len , fhr) ) != -1 )
	{
		...
		if ( number_of_columns == 2 )
			...
			curr_position += strtol (split_on_tab[0] ,
					&convert_to_int_temp ,
					10);
		...
		convertToAlignmentPairedEnded (sam_alignment_instance ,
				whole_genome ,
				split_on_tab ,
				split_on_dash ,
				split_on_comma ,
...
int main (int argc, char *argv[])
{
	...
	max_reads_in_each_line = strtol (argv[9] , &temp , 10);
