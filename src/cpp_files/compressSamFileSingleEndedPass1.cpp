...
void readAlignmentsAndCompress(char *input_samfilename, char *output_abridgefilename, char *reference_id, short int flag_ignore_soft_clippings, short int flag_ignore_mismatches, short int flag_ignore_unmapped_sequences)
{
	...
	Sam_Alignments prev_alignment;
	Sam_Alignments curr_alignment;
	Sam_Alignments temp_alignment;
	Sam_Alignments **alignment_pool_same_position;
	Compressed_DS **compressed_ds_pool;
	Reference_Sequence_Info reference_info;
	/********************************************************************/
	...
	compressed_ds_pool = new Compressed_DS*[MAX_POOL_SIZE];
	for (i = 0; i < MAX_POOL_SIZE; i++)
		compressed_ds_pool[i] = new Compressed_DS();
	...
	while ((line_len = getline(&line, &len, fhr)) != -1)
	{
		if (line[0] == '@')
		{
			if (line[1] == 'S' && line[2] == 'Q')
			{
				...
				//fflush(stdout);
				strcpy(reference_info.line[reference_info.number_of_reference_sequences], line);
				reference_info.number_of_reference_sequences++;
			}
		}
		else break;
	}
	...
	do
	{
		...
		curr_alignment.populateSamAlignmentInstance(split_line, number_of_fields, split_tags);
		...
		//cout<<curr_alignment;
		curr_alignment.generateIntegratedCigar(flag_ignore_soft_clippings, flag_ignore_mismatches, flag_ignore_unmapped_sequences);
		...
		if (strlen(prev_reference_name) == 0) // 1st chromosome - initialize stuffs
		{
			...
			//fflush(stdout);
			fprintf(fhw, "%s", reference_info.line[reference_sequence_index]);
