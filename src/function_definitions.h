		...
		char samformatflag_replacer_characters[])
{
	...
	for (i = 0; i < icigar_items_instance_index; i++)
	{
		...
		{
			sam_alignment_instance->soft_clippings.left[left_soft_clip_index] =
		...
		switch (cigar_items_instance[i].def)
		{
			...
			case 'I':
				for (j = 0; j < cigar_items_instance[i].len; j++)
				{
					...
					{
						case 'A':
							seq[sequence_index_to_be_replaced] =
									insert_characters[0];
							break;
						case 'T':
							seq[sequence_index_to_be_replaced] =
		...
		short int run_diagnostics)
{
	...
	if (isSequenceSoftClipped(curr_alignment->cigar) == 1)
	{
				...
				i < curr_alignment->read_seq_len - right_soft_clip_point; i++)
		{
			...
			curr_alignment->soft_clips_removed_qual[j] =
					curr_alignment->qual[i];
			j++;
		}
	...
	if (flag_ignore_soft_clippings == 0) // DO NOT ignore soft clippings
	{
		if (left_soft_clip_point != 0 && right_soft_clip_point == 0)
		{
			...
			{
				int j = 0;
				for (i = 0; curr_alignment->soft_clippings.left_qual[i] != '\0';
						i++)
				{
					curr_alignment->temp[j++] =
							curr_alignment->soft_clippings.left[i];
					curr_alignment->temp[j++] =
							curr_alignment->soft_clippings.left_qual[i];
				}
			...
			else
			{
						...
						i++)
				{
					curr_alignment->temp[j++] =
							curr_alignment->soft_clippings.right[i];
					curr_alignment->temp[j++] =
							curr_alignment->soft_clippings.right_qual[i];
				}
				curr_alignment->temp[j] = '\0';
			}
		}
	...
	if (run_diagnostics == 1)
	{
		...
		strcpy(
				...
				curr_alignment->icigar);
		sam_alignment_instance_diagnostics->start_position =
		...
		if (strcmp(sam_alignment_instance_diagnostics->seq, curr_alignment->seq)
				!= 0)
		{
			printf("\nSEQ Mismatch");
			flag = 1;
		...
		struct Pass3_Compression_Symbol_icigar_Mapping **symbol_icigar_mapping)
{
	...
	for (i = 0; i < MAX_SYMBOLS_FOR_PASS3_COMPRESSION; i++)
	{
		...
		{
			symbol_icigar_mapping[symbol_icigar_mapping_index]->symbolic_icigar[0] =
					individual_symbols[i];
			symbol_icigar_mapping[symbol_icigar_mapping_index]->symbolic_icigar[1] =
					individual_symbols[j];
			symbol_icigar_mapping[symbol_icigar_mapping_index]->symbolic_icigar[2] =
					'\0';
...
//buffer = (char*) malloc(sizeof(char) * pow(2, 32));
	...
	while ((line_len = getline(&buffer, &len, fhr)) != -1)
	{
		...
		if (buffer[0] == '>')
		{
			...
			j = 0;
			for (i = 1; buffer[i] != 32; i++)
				whole_genome->reference_sequence_name[whole_genome->number_of_reference_sequences][j++] =
						buffer[i];
			whole_genome->reference_sequence_name[whole_genome->number_of_reference_sequences][j++] =
					'\0';
		}
		...
		struct Abridge_Index *genome_index)
{
	...
	single_genome_sequence->reference_sequence_name = (char**) malloc(
			sizeof(char*) * MAX_REFERENCE_SEQUENCES);
	single_genome_sequence->reference_sequence_length =
...
//fflush (stdout);
	...
	single_genome_sequence->reference_sequence_length[0] =
			genome_index->end_byte[i] - genome_index->start_byte[i];
	fclose(fhr);
}
		...
		char **split_line)
{
	...
	while ((line_len = getline(&line, &len, fhr)) != -1)
	{
		...
		genome_index->start_byte[genome_index->number_of_items] = strtol(
				...
				&temp,
				10);
		genome_index->end_byte[genome_index->number_of_items] =
				genome_index->start_byte[genome_index->number_of_items]
		...
		short int *flag_ignore_alignment_scores)
{
	...
	while ((line_len = getline(&line, &len, fhr)) != -1)
	{
		...
		if (line_num == 1)
		{
			...
			*flag_ignore_all_quality_scores = strtol(split_line[4], &temp, 10);
...
//printf ("\nicigar %s" , icigar);
	for (i = 0; i < icigar_length; i++)
	{
		if ((icigar[i] != 'a' && icigar[i] != 't' && icigar[i] != 'g'
				...
				&& strchr(samformatflag_replacer_characters, icigar[i]) != NULL)
		{
			/*
	...
	for (i = 0; i < sam_alignment_instance->number_of_cigar_items; i++)
	{
		...
		else if (sam_alignment_instance->cigar_items[i].def == 'M')
		{
			for (j = distance_from_start_pos;
					...
					j++)
			{
				read_from_genome[read_from_genome_index++] =
						whole_genome->nucleotides[chromosome_index][sam_alignment_instance->start_position
								+ j - 1];
				read_from_genome_including_deletions[read_from_genome_including_deletions_index++] =
						whole_genome->nucleotides[chromosome_index][sam_alignment_instance->start_position
								+ j - 1];
				/*
