	...
	for ( i = 0 ; i < sam_alignment_instance->number_of_cigar_items ; i++ )
	{
		...
		else if ( sam_alignment_instance->cigar_items[i].def == 'M' )
		{
					...
					j++ )
			{
				...
				printf ("\nPosition %d Length of genome %d read_from_genome_including_deletions_index %d" , sam_alignment_instance->start_position + j - 1 , strlen (whole_genome->nucleotides[chromosome_index]) , read_from_genome_including_deletions_index);
