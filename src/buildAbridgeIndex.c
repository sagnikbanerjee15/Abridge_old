...
long long int findEndPointOfiCIGAR(char *icigar)
{
	...
	for (k = j; k >= 0; k--)
		if (icigar[k] != 'A' && icigar[k] != 'T' && icigar[k] != 'G' && icigar[k] != 'C' && icigar[k] != 'N') break;
	end = k;
	...
	*end = -1;
	...
	for (i = 0; i < number_of_fields; i++)
	{
		...
		if (strlen(split_icigar_and_num_reads[0]) != 1)
		{
			...
			if (*start + findEndPointOfiCIGAR(split_icigar_and_num_reads[0]) - 1 > *end) *end = *start + findEndPointOfiCIGAR(split_icigar_and_num_reads[0]) - 1;

		}
	}
}

void findContinousClusters(char *input_filename, char *output_filename)
{
	/*
	 *
	 */
...
int main(int argc, char *argv[])
{
	...
	findContinousClusters(input_filename, output_filename);
	return 0;
}
