		...
		char *dictionary_name)
{
	...
	int ROWS_split_on_tab = 10; //Reads the first line containing flags DO NOT CHANGE
	...
	short int flag_save_alignment_scores;
	...
	{
		printf(
				"Error! File abridge_index_filename = %s not found",
	...
	if (single == 0)
	{
		...
		{
			printf(
					"Error! File dictionary_name = %s not found",
					dictionary_name);
			exit(1);
		}
	}
	...
	for (i = 0; i < ROWS_split_on_comma; i++)
		split_on_newline[i] = (char*) malloc(
				sizeof(char) * COLS_split_on_newline);

	abridge_index = allocateMemoryAbridge_Index();
	last_cigar = (char*) malloc(sizeof(char) * 1000);
	last_cigar[0] = '\0';
	if (strcmp(dictionary_name, "dummy") == 0)
	...
	flag_ignore_all_quality_scores = strtol(
			...
			&convert_to_int_temp,
			10);
	flag_save_alignment_scores = strtol(
			split_on_tab[6],
			&convert_to_int_temp,
			10);
	...
	{
		length_of_continuous_segment = abridge_index->end[i]
				- abridge_index->start[i] + 1;
		max_bytes_for_current_index_entry = abridge_index->end_byte[i]
				- abridge_index->start_byte[i];
		number_of_bytes_read_from_compressed_file = 0;
		if (coverage_array != NULL)
			free(coverage_array);
		coverage_array = (long long int*) malloc(
				sizeof(long long int) * length_of_continuous_segment);
		for (j = 0; j < length_of_continuous_segment; j++)
			coverage_array[j] = 0;
		while (number_of_bytes_read_from_compressed_file
				< max_bytes_for_current_index_entry)
		{
			...
			if (buffer_for_pass1[0] == '@')
			{
				...
				{
					if (i != 0
							&& prev_stopping_location + 1 - 1
									!= last_location_of_current_chromosome) // Not the first chromosome
						printf(
								"\n%s\t%d\t%d\t%d",
								abridge_index->chromosome[i - 1],
								prev_stopping_location + 1 - 1,
								last_location_of_current_chromosome,
								0);
				}
				/***********************************************************************************/
				...
				last_location_of_current_chromosome = strtol(
						split_on_comma[1],
						&temp,
						10);
				continue;
			...
			if (max_reads_in_each_line == -1
					&& strstr(buffer_for_pass1, "brdg_"))
				number_of_columns--;
			...
			printf(
					"\ni=%d max_reads_in_each_line %d dictionary name %s number_of_columns %d line %s",
					i,
			...
			for (j = 0; j < number_of_distinct_cigars_in_a_line; j++)
			{
				...
				for (k = 0; k < num_of_cigar_types; k++)
				{
					...
					if (cigar_items_instance[k].def == 'a'
							|| cigar_items_instance[k].def == 't'
							|| cigar_items_instance[k].def == 'g'
							|| cigar_items_instance[k].def == 'c'
							|| cigar_items_instance[k].def == 'n') // Soft clips
						continue;
					else if (cigar_items_instance[k].def >= (33 + 90)
							&& cigar_items_instance[k].def <= (73 + 90)) // Quality scores
						continue;
					...
					else if (cigar_items_instance[k].def == '!'
							|| cigar_items_instance[k].def == '"'
							|| cigar_items_instance[k].def == '#'
							|| cigar_items_instance[k].def == '$'
							|| cigar_items_instance[k].def == '%') // Insertions in reads
						continue;
					else if (cigar_items_instance[k].def == '&'
							|| cigar_items_instance[k].def == '\''
							|| cigar_items_instance[k].def == '('
					 ...
					 */
					if (generate_overlapping_coverage == 1
							&& generate_nonoverlapping_coverage == 0)
					{
						//printf ("\n%d %c" , cigar_items_instance[k].len , cigar_items_instance[k].def);
						while (cigar_items_instance[k].len--)
						{
							//printf ("\nUpdating coverage");
							if (curr_position - abridge_index->start[i] + l < 0
									|| curr_position - abridge_index->start[i]
											+ l >= length_of_continuous_segment)
							{
								printf("\nIncorrect");
								printf(
										"\ni=%d %d %d",
										...
										curr_position - abridge_index->start[i]
												+ l,
										length_of_continuous_segment);
								printf(
										"\nCurrent position=%d, abrdige_start=%d l=%d",
										curr_position,
										abridge_index->start[i],
										l);
								printf(
										"\niCIGAR: %s j = %d k = %d",
										split_on_comma[j],
							...
							coverage_array[curr_position
									- abridge_index->start[i] + l] +=
									number_of_repititions_of_the_same_reads;
							l++;
						}
					}
					else if (generate_overlapping_coverage == 0
							&& generate_nonoverlapping_coverage == 1)
						coverage_array[curr_position - abridge_index->start[i]] +=
								number_of_repititions_of_the_same_reads;
				}
				/*
				 if ( abridge_index->start[i] == 23046848 )
		...
		else if (d == 0 && (bg == 1 || bga == 1))
		{
			...
			for (j = 0; j < length_of_continuous_segment; j++)
			{
				...
				else
				{
					...
					{
						if (abridge_index->start[i] + local_start - 1
								!= abridge_index->start[i] + local_end)
						{
							if ((bg == 1 && previous_coverage != 0) || bga == 1)
								printf(
										"\n%s\t%d\t%d\t%d",
										abridge_index->chromosome[i],
										abridge_index->start[i] + local_start
												- 1,
										abridge_index->start[i] + local_end,
										previous_coverage);
						}
				...
				if (abridge_index->start[i] + local_start - 1
					...
					if ((bg == 1 && previous_coverage != 0) || bga == 1)
						printf(
								"\n%s\t%d\t%d\t%d",
								abridge_index->chromosome[i],
								abridge_index->start[i] + local_start - 1,
								abridge_index->start[i] + local_end,
								previous_coverage);
			}
			if (coverage_array[length_of_continuous_segment - 1]
					!= coverage_array[length_of_continuous_segment - 2]) // The last nucleotide was a mismatch
			{
				if (abridge_index->start[i] + length_of_continuous_segment - 2
						!= abridge_index->start[i]
								+ length_of_continuous_segment - 1)
					if ((bg == 1 && previous_coverage != 0) || bga == 1)
						printf(
								"\n%s\t%d\t%d\t%d",
								abridge_index->chromosome[i],
								abridge_index->start[i]
										+ length_of_continuous_segment - 2,
								abridge_index->start[i]
										+ length_of_continuous_segment - 1,
								previous_coverage);
			}
...
int main(int argc, char *argv[])
{
	...
	generateCoverageFromCompressedMappedFile(
			pass1_filename,
			abridge_index_filename,
			d,
			bg,
			...
			generate_nonoverlapping_coverage,
			single,
			max_reads_in_each_line,
			dictionary_name);
