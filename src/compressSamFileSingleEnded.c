...
static error_t parse_opt (int key, char *arg, struct argp_state *state)
{
	...
	switch ( key )
	{
		...
		case ARGP_KEY_END:
			...
			if ( strcmp (arguments->input_sam_filename , "") == 0 || strcmp (arguments->output_abridge_filename ,
					"") == 0 || strcmp (arguments->genome_filename , "") == 0 || strcmp (arguments->unmapped_filename ,
					"") == 0 || strcmp (arguments->name_of_file_with_max_commas ,
	...
	for ( i = 0 ; i < compressed_ds_pool_total ; i++ )
	{
		...
		if ( flag_ignore_quality_scores_for_matched_bases == 0 ) //Write out the entire quality score
		{
			for ( j = 0 ; j < compressed_ds_pool[i]->num_reads ; j++ )
			{
				...
				splitCigar (compressed_ds_pool[i]->cigar ,
						&num_of_types ,
						cigar_items_instance);
				for ( m = 0 ; m < num_of_types ; m++ )
					if ( cigar_items_instance[m].def != 'D' && cigar_items_instance[m].def != 'N' )
		...
		struct Cigar_Items *cigar_items_instance)
{
	...
	/********************************************************************/
	for ( i = 0 ; i < compressed_ds_pool_index ; i++ )
		prepareIcigarForComparison (modified_icigars[i] ,
				compressed_ds_pool[i]->icigar);
	...
	for ( i = 0 ; i < compressed_ds_pool_index ; i++ )
	{
		...
		strcpy(compressed_ds_pool_rearranged[compressed_ds_pool_rearranged_index]->icigar ,
				compressed_ds_pool[i]->icigar);
		strcpy(compressed_ds_pool_rearranged[compressed_ds_pool_rearranged_index]->cigar ,
				compressed_ds_pool[i]->cigar);
	...
	{
		int cigar_length = 0;
		splitCigar (compressed_ds_pool_rearranged[i]->cigar ,
				&num_of_types ,
				cigar_items_instance);
		for ( m = 0 ; m < num_of_types ; m++ )
			cigar_length += cigar_items_instance[m].len;
		...
		short int flag_ignore_alignment_scores)
{
	...
	fhw_name_of_file_with_max_commas = fopen (name_of_file_with_max_commas ,
			"w");
	...
	while ( ( line_len = getline ( &line , &len , fhr) ) != -1 )
	{
		if ( line[0] == '@' )
		{
			if ( line[1] == 'S' && line[2] == 'Q' )
			{
				...
				//fflush(stdout);
				strcpy(reference_info[number_of_reference_sequences]->line ,
						line);
				splitByDelimiter (line , '\t' , split_line);
				splitByDelimiter (split_line[1] , ':' , split_tags);
				//printf ("\nLoading chromosome %s" , split_tags[1]);
				strcpy(reference_info[number_of_reference_sequences]->reference_name ,
						split_tags[1]);
	...
	do
	{
		...
		populateSamAlignmentInstance (curr_alignment ,
				split_line ,
				number_of_fields ,
				split_tags);
		...
		{
			strcpy(curr_alignment->tags[curr_alignment->number_of_tag_items].name ,
					"NH");
			strcpy(curr_alignment->tags[curr_alignment->number_of_tag_items].type ,
					"i");
			strcpy(curr_alignment->tags[curr_alignment->number_of_tag_items].val ,
					split_line[2]);
		...
		generateIntegratedCigarSingleEnded (curr_alignment ,
		...
		if ( strlen (prev_reference_name) == 0 ) // 1st chromosome - initialize stuffs
		{
			...
			strcpy(compressed_ds_pool[compressed_ds_pool_index]->icigar ,
					curr_alignment->icigar);
			strcpy(compressed_ds_pool[compressed_ds_pool_index]->cigar ,
			...
			//fflush(stdout);
			reference_sequence_index = findChromosomeIndex (reference_info ,
					prev_reference_name ,
					number_of_reference_sequences);
			fprintf (fhw_pass1 ,
					"%s" ,
		...
		else if ( strcmp (prev_reference_name , curr_reference_name) != 0 ) // New chromosome
		{
			...
			//fflush(stdout);
			reModeliCIGARSSingleEnded (compressed_ds_pool ,
		...
		else // Same chromosome
		{
			...
			if ( previous_position == current_position )
			{
				...
				for ( i = 0 ; i < compressed_ds_pool_index ; i++ )
				{
					if ( strcmp (compressed_ds_pool[i]->icigar ,
							curr_alignment->icigar) == 0 )
					{
						compressed_ds_pool[i]->num_reads++;
						strcpy(qual_scores[quality_score_index] ,
								curr_alignment->qual);
						strcpy(read_names[quality_score_index] ,
...
int main (int argc, char *argv[])
{
	 ...
	 */
	readAlignmentsAndCompress (name_of_file_with_quality_scores ,
