		...
		char *qual)
{
	...
	for ( i = 0 ; i < compressed_ds_pool_total ; i++ )
	{
		...
		if ( flag_ignore_all_quality_scores == 0 && flag_ignore_quality_scores_for_matched_bases == 0 )
		{
			for ( j = 0 ; j < compressed_ds_pool[i]->num_reads ; j++ )
			{
				/*
		...
		short int flag_ignore_alignment_scores)
{
	...
	if ( fhw_qual == NULL )
	{
		printf ("6. %s File cannot be created" ,
...
int main (int argc, char *argv[])
{
	...
	strcpy(name_of_file_with_quality_scores ,
			arguments.name_of_file_with_quality_scores);
	strcpy(frequency_of_flags_filename , arguments.frequency_of_flags_filename);
	...
	compressPairedEndedAlignments (frequency_of_flags_filename ,
			name_of_file_with_quality_scores ,
